...
//
    ...
    status = PCD_Request(PICC_REQALL,CT);//寻卡
    
//    printf("\r\nstatus>>>>>>%d\r\n", status);
    
    if(status==PCD_OK)//寻卡成功
    {
         printf("\r\n寻卡成功\r\n");
         status=PCD_ERR;
         status = PCD_Anticoll(SN);//防冲撞	       
    }
 
    if (status==PCD_OK)//防衝撞成功
    {
        status=PCD_ERR;		
        ShowID(SN); // 串口打印卡的ID号
        
        status =PCD_Select(SN);
    ...
    if(status==PCD_OK)//選卡成功
    {
 
        status=PCD_ERR;
        status =PCD_AuthState(0x60,0x09,KEY,SN);
     }
    if(status==PCD_OK)//驗證成功
    {
        status=PCD_ERR;
        status=PCD_ReadBlock(s,RFID);
    }
 
    if(status==PCD_OK)//讀卡成功
    {
        status=PCD_ERR;
        osDelay(100);
    }
